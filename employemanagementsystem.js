...
// Simple internship assignment - manage a list of employees with basic CRUD-ish operations
// keeping all employees in one array, acts like our "database" for this assignment
let employees = [];
// just using a counter instead of random ids, easier to track while testing
let nextId = 1;
function addEmployee(name, department, salary, isActive = true) {
    const newEmployee = {
        id: nextId,
        name,
        department,
...
// returns only employees that are currently active
...
function getHighestPaidEmployee() {
    if (employees.length === 0)
        return undefined;
    // simple loop, could use sort but this is fine for small lists
